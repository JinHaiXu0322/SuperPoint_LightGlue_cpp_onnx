...
static void printUsage(const char* prog) {
    std::cout
        ...
        << "  " << prog << " <sp.onnx> <lg.onnx> <img0> <img1> <out.png> <H> <W> [iters] [--cuda] [--device_id N] [--profile]\n\n"
        << "Usage (resident interactive):\n"
        << "  " << prog << " <sp.onnx> <lg.onnx> <H> <W> [iters] [warmup] [--cuda] [--device_id N] [--profile]\n"
        << "  then input lines: <img0> <img1> [out.png], or 'q' to quit\n";
}

// 新增：解析可选参数（简单实现：支持 --cuda / --device_id N / --profile）
struct ExtraFlags {
    bool use_cuda = false;
    int device_id = 0;
...
static ExtraFlags parseExtraFlags(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if (a == "--cuda" || a == "--use_cuda") {
            f.use_cuda = true;
        } else if (a == "--device_id" || a == "--device") {
            if (i + 1 < argc) {
                int v = 0;
                if (tryParseInt(argv[i + 1], v)) {
                    f.device_id = v;
                    i++;
                }
            }
        } else if (a == "--profile" || a == "--profiling") {
...
int main(int argc, char** argv) {
    ...
    // 如果用户指定 --cuda，则尝试启用 CUDA EP；失败自动回退 CPU
    bool use_cuda = false;
    if (flags.use_cuda) {
        use_cuda = tryEnableCuda(opt, flags.device_id);
    }
    ...
    // New: print requested vs enabled CUDA status and available providers again
    std::cout << "[EP] requested_use_cuda=" << (flags.use_cuda ? 1 : 0)
    ...
    std::cout << "[Config] mode=" << (one_shot ? "one-shot" : "resident")
              ...
              << " use_cuda=" << (use_cuda ? 1 : 0) << " device_id=" << flags.device_id

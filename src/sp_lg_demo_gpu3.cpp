...
// 方案三：先全局按 score 预筛，再做网格均匀化
// 推荐参数（针对 752x480 输入）：pre_top_n=1200, cell_size=48, per_cell_topk=4
static void filterFeaturesPreTopNThenGrid(SPFeatures& f, int img_h, int img_w,
                                          int pre_top_n, int cell_size, int per_cell_topk) {
    if (f.N <= 1 || f.D <= 0) return;
    if (pre_top_n <= 0 || cell_size <= 0 || per_cell_topk <= 0) return;
    ...
    std::cout << "[Filter][PreTopN+Grid] pre_top_n=" << pre_top_n
              << " cell_size=" << cell_size
...
static void printUsage(const char* prog) {
    std::cout
        << "Usage:\n"
        << "  One-shot mode:\n"
        << "    " << prog << " --oneshot <sp.onnx> <lg.onnx> <img0> <img1> <out.png> <H> <W> [iters] [pre_top_n] [cell_size] [per_cell_topk] [--profile]\n"
        << "\n"
        << "  Resident interactive mode:\n"
        << "    " << prog << " --resident <sp.onnx> <lg.onnx> <H> <W> [iters] [warmup] [pre_top_n] [cell_size] [per_cell_topk] [--profile]\n"
...
int main(int argc, char** argv) {
    ...
    std::cout << "[Config] mode=" << (one_shot ? "one-shot" : "resident")
              ...
              << " filter=PreTopNThenGrid(pre_top_n=" << pre_top_n << ",cell_size=" << cell_size << ",per_cell_topk=" << per_cell_topk << ")"
    ...
    auto run_pair = [&](const cv::Mat& img0, const cv::Mat& img1, const std::string& out_p) {
        ...
        for (int iter = 1; iter <= iters; ++iter) {
            ...
            filterFeaturesPreTopNThenGrid(f0, H, W, pre_top_n, cell_size, per_cell_topk);
            filterFeaturesPreTopNThenGrid(f1, H, W, pre_top_n, cell_size, per_cell_topk);

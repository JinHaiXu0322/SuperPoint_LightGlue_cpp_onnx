...
// 方案五：全局 Top-N 预筛 + 网格 Top-K + 最终总数截断
// 推荐参数（针对 752x480 输入）：pre_top_n=1500, cell_size=40, per_cell_topk=5, max_total_n=768
static void filterFeaturesPreTopNGridTopKFinalCap(SPFeatures& f, int img_h, int img_w,
                                                   int pre_top_n, int cell_size,
                                                   int per_cell_topk, int max_total_n) {
    if (f.N <= 1 || f.D <= 0) return;
    if (pre_top_n <= 0 || cell_size <= 0 || per_cell_topk <= 0 || max_total_n <= 0) return;
    if (f.desc.size() < (size_t)f.N * (size_t)f.D) return;

    auto pre = selectTopByScore(f, pre_top_n);

    int cols = (img_w + cell_size - 1) / cell_size;
    int rows = (img_h + cell_size - 1) / cell_size;
    int bucket_num = rows * cols;
    std::vector<std::vector<int>> buckets((size_t)bucket_num);

    for (int idx : pre) {
        const auto& kp = f.kpts[(size_t)idx];
        int cx = std::max(0, std::min(cols - 1, (int)(kp.pt.x / cell_size)));
        int cy = std::max(0, std::min(rows - 1, (int)(kp.pt.y / cell_size)));
        int bid = cy * cols + cx;
        if ((int)buckets[(size_t)bid].size() < per_cell_topk) {
            buckets[(size_t)bid].push_back(idx);
        }
    }

    std::vector<int> selected;
    selected.reserve((size_t)bucket_num * (size_t)per_cell_topk);
    for (auto& b : buckets) {
        for (int idx : b) selected.push_back(idx);
    }
    selected = sortSelectedByScoreDesc(f, std::move(selected));
    if ((int)selected.size() > max_total_n) selected.resize((size_t)max_total_n);

    int64_t oldN = f.N;
    applySelectedIndices(f, selected);
    std::cout << "[Filter][PreTopN+GridTopK+Cap] pre_top_n=" << pre_top_n
              << " cell_size=" << cell_size
              << " per_cell_topk=" << per_cell_topk
...
static void printUsage(const char* prog) {
    std::cout
        << "Usage:\n"
        << "  One-shot mode:\n"
        << "    " << prog << " --oneshot <sp.onnx> <lg.onnx> <img0> <img1> <out.png> <H> <W> [iters] [pre_top_n] [cell_size] [per_cell_topk] [max_total_n] [--profile]\n"
        << "\n"
        << "  Resident interactive mode:\n"
        << "    " << prog << " --resident <sp.onnx> <lg.onnx> <H> <W> [iters] [warmup] [pre_top_n] [cell_size] [per_cell_topk] [max_total_n] [--profile]\n"
...
int main(int argc, char** argv) {
    ...
    if (one_shot) {
        ...
        if (argc >= 11) {
            int v = 0;
            if (tryParseInt(argv[10], v) && v > 0) pre_top_n = v;
        }
        if (argc >= 12) {
            int v = 0;
            if (tryParseInt(argv[11], v) && v > 0) cell_size = v;
        }
        if (argc >= 13) {
            int v = 0;
            if (tryParseInt(argv[12], v) && v > 0) per_cell_topk = v;
        }
        if (argc >= 14) {
            int v = 0;
            if (tryParseInt(argv[13], v) && v > 0) max_total_n = v;
        }
        warmup = 0;
    } else {
        if (argc < 6) {
        ...
        if (argc >= 9) {
            int v = 0;
            if (tryParseInt(argv[8], v) && v > 0) pre_top_n = v;
        }
        if (argc >= 10) {
            int v = 0;
            if (tryParseInt(argv[9], v) && v > 0) cell_size = v;
        }
        if (argc >= 11) {
            int v = 0;
            if (tryParseInt(argv[10], v) && v > 0) per_cell_topk = v;
        }
        if (argc >= 12) {
            int v = 0;
            if (tryParseInt(argv[11], v) && v > 0) max_total_n = v;
        }
    }
    ...
    std::cout << "[Config] mode=" << (one_shot ? "one-shot" : "resident")
              ...
              << " filter=PreTopN+GridTopK+Cap(pre_top_n=" << pre_top_n << ",cell_size=" << cell_size << ",per_cell_topk=" << per_cell_topk << ",max_total_n=" << max_total_n << ")"
    ...
    auto run_pair = [&](const cv::Mat& img0, const cv::Mat& img1, const std::string& out_p) {
        ...
        for (int iter = 1; iter <= iters; ++iter) {
            ...
            filterFeaturesPreTopNGridTopKFinalCap(f0, H, W, pre_top_n, cell_size, per_cell_topk, max_total_n);
            filterFeaturesPreTopNGridTopKFinalCap(f1, H, W, pre_top_n, cell_size, per_cell_topk, max_total_n);

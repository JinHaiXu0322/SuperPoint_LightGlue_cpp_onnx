...
// 方案一：网格分桶 + 每格 Top-K
// 推荐参数（针对 752x480 输入）：cell_size=48, per_cell_topk=4
static void filterFeaturesGridTopK(SPFeatures& f, int img_h, int img_w, int cell_size, int per_cell_topk) {
    if (f.N <= 1 || f.D <= 0) return;
    if (cell_size <= 0 || per_cell_topk <= 0) return;
    ...
    std::cout << "[Filter][GridTopK] cell_size=" << cell_size
...
static void printUsage(const char* prog) {
    std::cout
        << "Usage:\n"
        << "  One-shot mode:\n"
        << "    " << prog << " --oneshot <sp.onnx> <lg.onnx> <img0> <img1> <out.png> <H> <W> [iters] [cell_size] [per_cell_topk] [--profile]\n"
        << "\n"
        << "  Resident interactive mode:\n"
        << "    " << prog << " --resident <sp.onnx> <lg.onnx> <H> <W> [iters] [warmup] [cell_size] [per_cell_topk] [--profile]\n"
...
int main(int argc, char** argv) {
    ...
    if (one_shot) {
        ...
        if (argc >= 11) {
            int v = 0;
            if (tryParseInt(argv[10], v) && v > 0) cell_size = v;
        }
        if (argc >= 12) {
            int v = 0;
            if (tryParseInt(argv[11], v) && v > 0) per_cell_topk = v;
        }
        warmup = 0;
    } else {
        if (argc < 6) {
        ...
        if (argc >= 9) {
            int v = 0;
            if (tryParseInt(argv[8], v) && v > 0) cell_size = v;
        }
        if (argc >= 10) {
            int v = 0;
            if (tryParseInt(argv[9], v) && v > 0) per_cell_topk = v;
        }
    }
    ...
    std::cout << "[Config] mode=" << (one_shot ? "one-shot" : "resident")
              ...
              << " filter=GridTopK(cell_size=" << cell_size << ",per_cell_topk=" << per_cell_topk << ")"
    ...
    auto run_pair = [&](const cv::Mat& img0, const cv::Mat& img1, const std::string& out_p) {
        ...
        for (int iter = 1; iter <= iters; ++iter) {
            ...
            filterFeaturesGridTopK(f0, H, W, cell_size, per_cell_topk);
            filterFeaturesGridTopK(f1, H, W, cell_size, per_cell_topk);

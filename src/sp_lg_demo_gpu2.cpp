...
// 方案二：ANMS（Adaptive Non-Maximal Suppression）
// 推荐参数（针对 752x480 输入）：max_kpts=768, robust_coeff=0.90
static void filterFeaturesANMS(SPFeatures& f, int max_kpts, float robust_coeff) {
    ...
    auto order = makeScoreOrderDesc(f);
    std::vector<float> radius((size_t)f.N, std::numeric_limits<float>::max());

    for (int rank_i = 1; rank_i < (int)order.size(); ++rank_i) {
        int idx_i = order[(size_t)rank_i];
        const auto& pi = f.kpts[(size_t)idx_i].pt;
        float si = getScoreAt(f, idx_i);
        float best_r2 = std::numeric_limits<float>::max();

        for (int rank_j = 0; rank_j < rank_i; ++rank_j) {
            int idx_j = order[(size_t)rank_j];
            float sj = getScoreAt(f, idx_j);
            if (sj <= si * robust_coeff) continue;
            const auto& pj = f.kpts[(size_t)idx_j].pt;
            float dx = pi.x - pj.x;
            float dy = pi.y - pj.y;
            float d2 = dx * dx + dy * dy;
            if (d2 < best_r2) best_r2 = d2;
        }
        radius[(size_t)idx_i] = std::sqrt(best_r2);
    }

    int strongest = order[0];
    radius[(size_t)strongest] = std::numeric_limits<float>::max();

    std::vector<int> selected((size_t)f.N);
    for (int i = 0; i < (int)f.N; ++i) selected[(size_t)i] = i;
    std::sort(selected.begin(), selected.end(), [&](int a, int b) {
        float ra = radius[(size_t)a];
        float rb = radius[(size_t)b];
        if (ra == rb) {
            float sa = getScoreAt(f, a);
            float sb = getScoreAt(f, b);
            if (sa == sb) return a < b;
    ...
    std::cout << "[Filter][ANMS] max_kpts=" << max_kpts
...
static void printUsage(const char* prog) {
    std::cout
        << "Usage:\n"
        << "  One-shot mode:\n"
        << "    " << prog << " --oneshot <sp.onnx> <lg.onnx> <img0> <img1> <out.png> <H> <W> [iters] [max_kpts] [robust_coeff] [--profile]\n"
        << "\n"
        << "  Resident interactive mode:\n"
        << "    " << prog << " --resident <sp.onnx> <lg.onnx> <H> <W> [iters] [warmup] [max_kpts] [robust_coeff] [--profile]\n"
...
int main(int argc, char** argv) {
    ...
    if (one_shot) {
        ...
        if (argc >= 12) {
            float v = 0.f;
            if (tryParseFloat(argv[11], v) && v > 0.f && v <= 1.f) robust_coeff = v;
        }
        warmup = 0;
    } else {
        if (argc < 6) {
        ...
        if (argc >= 10) {
            float v = 0.f;
            if (tryParseFloat(argv[9], v) && v > 0.f && v <= 1.f) robust_coeff = v;
        }
    }
    ...
    std::cout << "[Config] mode=" << (one_shot ? "one-shot" : "resident")
              ...
              << " filter=ANMS(max_kpts=" << max_kpts << ",robust_coeff=" << robust_coeff << ")"
    ...
    auto run_pair = [&](const cv::Mat& img0, const cv::Mat& img1, const std::string& out_p) {
        ...
        for (int iter = 1; iter <= iters; ++iter) {
            ...
            filterFeaturesANMS(f0, max_kpts, robust_coeff);
            filterFeaturesANMS(f1, max_kpts, robust_coeff);

...
static void filterFeaturesByMinDistance(SPFeatures& f, float min_dist) {
    ...
    std::sort(order.begin(), order.end(), [&](int a, int b) {
        float sa = ((size_t)a < f.scores.size()) ? f.scores[(size_t)a] : 0.f;
        float sb = ((size_t)b < f.scores.size()) ? f.scores[(size_t)b] : 0.f;
        if (sa == sb) return a < b;
        return sa > sb;
    });
    ...
    std::vector<int> kept;
    kept.reserve((size_t)f.N);

    for (int idx : order) {
        const auto& kp = f.kpts[(size_t)idx];
        int cx = (int)std::floor(kp.pt.x / cell);
        int cy = (int)std::floor(kp.pt.y / cell);
    ...
    if (kept.size() == (size_t)f.N) return;
    ...
    new_desc.reserve(kept.size() * (size_t)f.D);

    for (int idx : kept) {
        new_kpts.push_back(f.kpts[(size_t)idx]);
        new_scores.push_back(((size_t)idx < f.scores.size()) ? f.scores[(size_t)idx] : 0.f);
    ...
    std::cout << "[Filter] min_kpt_dist=" << min_dist << " keep " << f.N << "/" << oldN << " keypoints\n";
}
...
static void printUsage(const char* prog) {
    ...
    << "  " << prog << " <sp.onnx> <lg.onnx> <img0> <img1> <out.png> <H> <W> [iters] [min_kpt_dist] [--profile]\n\n"
        << "Usage (resident interactive):\n"
    << "  " << prog << " <sp.onnx> <lg.onnx> <H> <W> [iters] [warmup] [min_kpt_dist] [--profile]\n"
        << "  min_kpt_dist: -1 means disable distance filtering and keep all keypoints\n"
...
int main(int argc, char** argv) {
    ...
    if (argc >= 8) {
        ...
        if (tryParseInt(argv[6], Htmp) && tryParseInt(argv[7], Wtmp)) {
            ...
            if (argc >= 10) {
                float d = 0.f;
                if (tryParseFloat(argv[9], d) && d >= -1.f) min_kpt_dist = d;
            }
    ...
    if (!one_shot) {
        ...
        if (argc >= 8) {
            float d = 0.f;
            if (tryParseFloat(argv[7], d) && d >= -1.f) min_kpt_dist = d;
        }
    }
    ...
    auto run_pair = [&](const cv::Mat& img0, const cv::Mat& img1, const std::string& out_p) {
        ...
        for (int iter = 1; iter <= iters; ++iter) {
            ...
            filterFeaturesByMinDistance(f0, min_kpt_dist);
            filterFeaturesByMinDistance(f1, min_kpt_dist);
